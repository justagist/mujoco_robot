"""Cartesian impedance controller for end-effector tracking with torque control."""
...
class CartesianImpedanceController:
    """Simplified PD control for end-effector tracking (returns joint torques)."""
    ...
    def compute_cmd(self):
        ee_name = self._robot.ee_names[0]
        curr_pos, curr_ori = self._robot.get_link_pose(link_id=self._robot.get_link_id(ee_name))
        curr_joint_pos = self._robot.get_actuated_joint_positions()
        curr_joint_vel = self._robot.get_actuated_joint_velocities()

        jac = self._robot.get_jacobian(ee_link_name=ee_name)
        ...
        null_proj = np.eye(curr_joint_pos.size) - jac.T.dot(np.linalg.pinv(jac.T, rcond=1e-3))
        null_torque = null_proj.dot(self._null_kp * (self._nullspace_target - curr_joint_pos))
